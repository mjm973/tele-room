...
namespace Byn.Awrtc.Unity
{
    ...
    class UnityVideoCapturer : HLCustomVideoCapturer
    {
        ...
        {
            //Workaround for bug0011: Unity will stop webcam textures if the scene is changed
            if (mTexture.isPlaying == false)
            {
                Debug.LogWarning("WebcamTexture stopped playing? Try restart.");
                mTexture.Play();
            }
